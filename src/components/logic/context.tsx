...
type StreamingAvatarContextProps = {
  ...
  messages: Message[];
  toolsMessages: Message[]; // Nueva lista para mensajes de tools
...
const StreamingAvatarContext = React.createContext<StreamingAvatarContextProps>(
  {
    ...
    setStream: () => {},
    messages: [],
    toolsMessages: [], // Inicializar lista de tools
    appendMessage: () => {},
    clearMessages: () => {},
    handleUserTalkingMessage: () => {},
    handleStreamingTalkingMessage: () => {},
...
const useStreamingAvatarMessageState = () => {
  ...
  const [toolsMessages, setToolsMessages] = useState<Message[]>([]); // Estado separado para tools
  ...
  return {
    ...
    appendMessage: (sender: MessageSender, content: string) => {
      ...
      // Si es un mensaje de TOOLS, agregarlo a toolsMessages con verificación de duplicados
      if (sender === MessageSender.TOOLS) {
        setToolsMessages((prev) => {
          // Verificar si ya existe un mensaje de tools con el mismo contenido
          const messageExists = prev.some(
            (msg) =>
              msg.content === content && msg.sender === MessageSender.TOOLS
          );
        ...
        // Para otros mensajes, agregarlos a messages con verificación de duplicados
        setMessages((prev) => {
          const messageExists = prev.some(
